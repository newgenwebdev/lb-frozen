...
import type { PWPRule, PromoStatus } from "@/lib/types/promo";

export type PWPSortField = "displayId" | "rule" | "type" | "value" | "date" | "redemptions" | "status";
export type SortDirection = "asc" | "desc";

type PWPRulesTableProps = {
  rules: PWPRule[];
  isLoading?: boolean;
  sortField?: PWPSortField;
  sortDirection?: SortDirection;
  onSort?: (field: PWPSortField) => void;
...
export function PWPRulesTable({
  ...
  onEdit,
  onDelete,
}: PWPRulesTableProps): React.JSX.Element {
  ...
  const handleSort = (field: PWPSortField): void => {
    onSort?.(field);
  };
      ...
      <div className="overflow-hidden rounded-lg border border-[#E5E7EB] bg-white">
        <div className="p-12 text-center">
          ...
          <p className="font-public text-[14px] text-[#6A7282]">
            Get started by adding your first PWP rule
          </p>
        </div>
      </div>
    );
          ...
          <thead>
            <tr className="border-b border-[#E5E7EB] bg-[#F9FAFB]">
              ...
              <th className="px-3 py-3 text-left font-geist text-[12px] font-medium tracking-[-0.12px] text-[#6A7282]">
                ...
                >
                  <span>Rule</span>
                  <SortIcon field="rule" currentField={sortField} direction={sortDirection} />
                </button>
              </th>
              <th className="px-3 py-3 text-left font-geist text-[12px] font-medium tracking-[-0.12px] text-[#6A7282]">
                <button
              ...
              <th className="px-3 py-3 text-left font-geist text-[12px] font-medium tracking-[-0.12px] text-[#6A7282]">
                ...
                >
                  ...
                  <SortIcon field="redemptions" currentField={sortField} direction={sortDirection} />
                </button>
              </th>
              <th className="px-3 py-3 text-left font-geist text-[12px] font-medium tracking-[-0.12px] text-[#6A7282]">
                <button
          ...
          <tbody>
            {rules.map((rule) => (
              ...
              >
                ...
                <td className="px-3 py-4">
                  <span className="font-public text-[14px] font-medium text-[#030712]">
                    {rule.rewardType === "percentage" ? "%" : "fixed"}
                  </span>
                </td>
                <td className="px-3 py-4">
                  <span className="font-public text-[14px] font-medium text-[#030712]">
                ...
                <td className="px-3 py-4">
                  <StatusBadge status={rule.status} endsAt={rule.endsAt} />
                </td>
                <td className="py-4 pl-3 pr-6">
                  <button
                    onClick={(e) => handleMenuToggle(rule.id, e)}
