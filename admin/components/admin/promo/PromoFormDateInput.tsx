...
export function PromoFormDateInput({
  label,
  value,
  onChange,
  error,
  ...
  className = "",
}: PromoFormDateInputProps): React.JSX.Element {
  ...
  const handleSelectDate = (day: number): void => {
    const isoDate = formatDateToISO(currentMonth.getFullYear(), currentMonth.getMonth(), day);
    onChange(isoDate);
    setIsOpen(false);
  };
    ...
    <div className={className} ref={containerRef}>
      ...
      <div className="relative">
        ...
        <button
          ...
          className={`flex w-full cursor-pointer items-center rounded-lg border bg-white py-3 pl-12 pr-4 text-left font-public text-[14px] font-medium tracking-[-0.14px] outline-none transition-colors ${
            !value ? "text-[#99A1AF]" : "text-[#030712]"
          } ${
            error
        ...
        {isOpen && (
          <div className="absolute bottom-full left-0 z-50 mb-2 w-[280px] rounded-xl border border-[#E5E7EB] bg-white p-4 shadow-lg">
            ...
            <div className="mt-3 border-t border-[#E5E7EB] pt-3">
              <button
                ...
                onClick={() => {
                  const today = new Date();
                  const isoDate = formatDateToISO(today.getFullYear(), today.getMonth(), today.getDate());
                  onChange(isoDate);
                  setIsOpen(false);
                }}
