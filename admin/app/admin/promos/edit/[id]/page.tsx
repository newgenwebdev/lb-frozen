...
import { useUpdateCoupon, useUpdatePWPRule, useDeleteCoupon, useDeletePWPRule } from "@/lib/api/mutations";
import { useCoupon, usePWPRule } from "@/lib/api/queries";
...
// Helper to format ISO date to YYYY-MM-DD for form input
...
function EditPromoPageContent(): React.JSX.Element {
  ...
  // Fetch existing data
  const { data: couponData, isLoading: isCouponLoading } = useCoupon(isCoupon ? id : "");
  const { data: pwpData, isLoading: isPWPLoading } = usePWPRule(!isCoupon ? id : "");

  const isLoading = isCoupon ? isCouponLoading : isPWPLoading;
  ...
  // Populate coupon form when data is loaded
  useEffect(() => {
    if (couponData && isCoupon) {
      // Convert cents to dollars for fixed type
      const displayValue = couponData.type === "fixed" && couponData.value
        ? (couponData.value / 100).toString()
      ...
      setCouponForm({
        ...
        value: displayValue,
        startDate: formatDateForInput(couponData.starts_at),
        endDate: formatDateForInput(couponData.ends_at),
      });
    }
  }, [couponData, isCoupon]);

  // Populate PWP form when data is loaded
  useEffect(() => {
    if (pwpData && !isCoupon) {
      // Convert cents to dollars for cart value and fixed reward
      const displayCartValue = pwpData.trigger_cart_value
        ? (pwpData.trigger_cart_value / 100).toString()
        : "";
      const displayRewardValue = pwpData.reward_type === "fixed" && pwpData.reward_value
        ? (pwpData.reward_value / 100).toString()
        : pwpData.reward_value?.toString() || "";

      setPWPForm({
        name: pwpData.name || "",
        rule_description: pwpData.rule_description || "",
        trigger_type: pwpData.trigger_type || "product",
        trigger_product_id: pwpData.trigger_product_id || "",
        trigger_cart_value: displayCartValue,
        reward_product_id: pwpData.reward_product_id || "",
        reward_type: pwpData.reward_type || "percentage",
        reward_value: displayRewardValue,
        startDate: formatDateForInput(pwpData.starts_at),
        endDate: formatDateForInput(pwpData.ends_at),
      });
    }
  }, [pwpData, isCoupon]);
  ...
  const updatePWPRuleMutation = useUpdatePWPRule();
  const deleteCouponMutation = useDeleteCoupon();
  const deletePWPRuleMutation = useDeletePWPRule();

  const isSubmitting = updateCouponMutation.isPending || updatePWPRuleMutation.isPending;
  const isDeleting = deleteCouponMutation.isPending || deletePWPRuleMutation.isPending;
  ...
  const handleSubmit = async (): Promise<void> => {
    try {
      if (isCoupon) {
        ...
        await updatePWPRuleMutation.mutateAsync({
          id,
          data: {
            name: pwpForm.name,
        ...
        showToast("PWP rule updated successfully", "success");
      }
  ...
  const handleDelete = async (): Promise<void> => {
    const confirmed = await confirm({
      title: isCoupon ? "Delete Coupon" : "Delete PWP Rule",
      message: isCoupon
        ? "Are you sure you want to delete this coupon? This action cannot be undone."
        : "Are you sure you want to delete this PWP rule? This action cannot be undone.",
      confirmText: "Delete",
      cancelText: "Cancel",
      type: "danger",
    });
    ...
    try {
      if (isCoupon) {
        ...
        await deletePWPRuleMutation.mutateAsync(id);
        showToast("PWP rule deleted successfully", "success");
      }
      router.push("/admin/promos");
    } catch (error) {
      console.error("Failed to delete promo:", error);
      showToast("Failed to delete promo. Please try again.", "error");
    }
  };
...
export default function EditPromoPage(): React.JSX.Element {
  return (
